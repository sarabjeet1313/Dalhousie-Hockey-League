...
public class ParsingState implements IState {
	...
	public void nextState(StateContext context) {
		...
		context.setState(new CreateTeamState(input, output, initializedLeague, leagueDb, coachDb, configDb, managerDb));
	}

	public void doProcessing() {
		output.setOutput("Welcome to the Parsing State. It's time to parse the JSON and initialize your league.");
		output.sendOutput();
		InitializeLeagues leagueModel = new InitializeLeagues(filePath, leagueDb, output, input, coachDb, configDb, managerDb);
