	...
	public List<Player> getStrongestPlayers(Trade t, List<String> allTeamNameList, League league,
		...
		for (int j = 0; j < allTeamNameList.size(); j++) {
			...
			currentTeamPlayers = getPlayersOfSpecificType(requiredPlayerType,
					iTPInfoObject.getPlayersByTeam(allTeamNameList.get(j), league));
	...
	public League startTrade(League leagueObject) {
		...
		for (int i = 0; i < eligibleTeamNameList.size(); i++) {
			if (checkRandOfferChance(randOfferChance)) {
				...
				trade.setPlayerListOfferTeam(
						getWeakestPlayers(maxPlayersPerTrade, eligibleTeamNameList.get(i), leagueObject, ty, py));
				...
				if (ar.isAcceptOrReject(trade, leagueObject, randAcceptChance, true, py, ti)) {
					// iterate to teamPlayer list and make Swap 100% easier if directly changed to
					// database.
					...
					// call notify
					TradePublisher.getInstance().notify(trade.getTradeOfferTeam(), trade.getTradeRequestedTeam(),
							prepareToNotify(trade));
