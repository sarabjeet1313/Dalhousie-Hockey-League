...
public class InjuryCheckState implements ISimulationState {
    ...
    public InjuryCheckState(League leagueToSimulate, IInjuryManagement injury, ISchedule schedule,
                            InternalStateContext context, SeasonCalendar seasonCalendar, String currentDate, String endDate, int season, IUserOutput output, IStandingsPersistance standingsDb) {
    ...
    public ISimulationState nextState(InternalStateContext context) {
        if (schedule.anyUnplayedGame(currentDate)) {
            ...
            return new SimulateGameState(leagueToSimulate, schedule, standingsDb, context, seasonCalendar, currentDate, endDate, season, output);
        } else {
            if (seasonCalendar.isTradeDeadlinePending(this.currentDate)) {
                this.nextStateName = StateConstants.TRADING_STATE;
                return new TradingState(leagueToSimulate, trade, context, output, seasonCalendar, currentDate, endDate, season, standingsDb, schedule);
            } else {
                this.nextStateName = StateConstants.AGING_STATE;
                return new AgingState(leagueToSimulate, schedule, standingsDb, injury, context, seasonCalendar, currentDate, endDate, season, output);
            }
        }
    }
