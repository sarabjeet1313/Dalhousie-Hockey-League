...
public class InitializeLeagues implements IInitializeLeagues {
                ...
                while (teamListElement.hasNext()) {
                    ...
                    while (player_List.hasNext()) {
                        ...
                        if (isEmptyString(playerName)) {
                            output.setOutput(PlayerConstants.ENTER_DETAILS.toString() + count + PlayerConstants.EMPTY_MSG.toString());
                            output.sendOutput();
                            return null;
                        }
                        ...
                        if (isEmptyString(position)) {
                            output.setOutput(
                                    PlayerConstants.ENTER_PLAYER.toString() + count + PlayerConstants.POSITION_ERROR.toString());
                            output.sendOutput();
                            return null;
                        }

                        if (!(position.contains(GeneralConstants.FORWARD.toString()) || position.contains(GeneralConstants.GOALIE.toString())
                                || position.contains(GeneralConstants.DEFENSE.toString()))) {
                            output.setOutput(
                                    PlayerConstants.PLAYER.toString() + count + PlayerConstants.POSITION_TYPE_ERROR.toString());
                            output.sendOutput();
                            return null;
                        }
                        ...
                        if (age < 0) {
                            output.setOutput(PlayerConstants.PLAYER.toString() + count + PlayerConstants.PLAYER_AGE_ERROR.toString());
                            output.sendOutput();
                            return null;
                        }
    ...
    private List<Player> loadFreeAgentsInfo() {
        ...
        while (freeAgentElement.hasNext()) {
            ...
            if (isEmptyString(agentName)) {
                output.setOutput(PlayerConstants.ENTER_FREE_AGENT.toString() + count + CoachConstants.NAME.toString());
                output.sendOutput();
                return null;
            }
            ...
            if (isEmptyString(position)) {
                output.setOutput(PlayerConstants.FREE_AGENT.toString() + count + PlayerConstants.PLAYER_POSITION.toString());
                output.sendOutput();
                return null;
            }
            ...
            if (!(position.contains(GeneralConstants.FORWARD.toString()) || position.contains(GeneralConstants.GOALIE.toString())
                    || position.contains(GeneralConstants.DEFENSE.toString()))) {
                output.setOutput(PlayerConstants.FREE_AGENT.toString() + count + PlayerConstants.POSITION_TYPE_ERROR.toString());
                output.sendOutput();
                return null;
            }
            ...
            if (captain) {
                output.setOutput(PlayerConstants.FREE_AGENT.toString() + count + PlayerConstants.CANNOT_CAPTAIN.toString());
                output.sendOutput();
                return null;
            }
            ...
            if (age < 0) {
                output.setOutput(PlayerConstants.FREE_AGENT.toString() + count + PlayerConstants.PLAYER_AGE_ERROR.toString());
                output.sendOutput();
                return null;
            }
    ...
    private List<Coach> loadCoachesInfo() {
        ...
        while (coachElement.hasNext()) {
            ...
            if (isEmptyString(coachName)) {
                output.setOutput(CoachConstants.ENTER_COACH.toString() + coachCount + CoachConstants.NAME.toString());
                output.sendOutput();
                return null;
            }
            ...
            if (coachShooting < 0 || coachShooting > 1) {
                output.setOutput(CoachConstants.COACH.toString() + coachCount + CoachConstants.SHOOTING_ERROR.toString());
                output.sendOutput();
                return null;
            }
            ...
            if (coachChecking < 0 || coachChecking > 1) {
                output.setOutput(CoachConstants.COACH.toString() + coachCount + CoachConstants.CHECKING_ERROR.toString());
                output.sendOutput();
                return null;
            }
            ...
            if (coachSaving < 0 || coachSaving > 1) {
                output.setOutput(CoachConstants.COACH.toString() + coachCount + CoachConstants.SAVING_ERROR.toString());
                output.sendOutput();
                return null;
            }
    ...
    private GameResolver loadGameResolverInfo(JsonObject config) {
        ...
        double randomWinChance = gameResolverObj.get(GameConfigConstants.RANDOM_WIN_CHANCE.toString()).getAsDouble();
    ...
    private Injury loadInjuriesInfo(JsonObject config) {
        ...
        double randomInjuryChance = injuriesObj.get(GameConfigConstants.RANDOM_INJURY_CHANCE.toString()).getAsDouble();
    ...
    private Training loadTrainingInfo(JsonObject config) {
        ...
        int daysUntilStatIncreaseCheck = trainingObj.get(GameConfigConstants.STAT_INCREASE_CHECK.toString()).getAsInt();
    ...
    private Trading loadTradingInfo(JsonObject config) {
        ...
        double randomTradeOfferChance = tradingObj.get(GameConfigConstants.RANDOM_TRADE_OFFER_CHANCE.toString()).getAsDouble();
        ...
        double randomAcceptanceChance = tradingObj.get(GameConfigConstants.RANDOM_ACCEPTANCE_CHANCE.toString()).getAsDouble();
        if (randomTradeOfferChance < 0 || randomTradeOfferChance > 1) {
